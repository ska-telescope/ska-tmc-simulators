...
@test 'TERRAFORM: Valid against cpp src directory' {
    run make -f ../tests/Makefile TERRAFORM_LINT_TARGET=./src terraform-lint
    assert_success
}

@test 'TERRAFORM: Valid against docker src directory' {
    run make -f ../tests/Makefile TERRAFORM_LINT_TARGET=./images/ska-cicd-makefile terraform-lint
    assert_success
}

@test 'TERRAFORM: Valid against conan src directory' {
    run make -f ../tests/Makefile TERRAFORM_LINT_TARGET=./conan terraform-lint
    assert_success
}
...
@test 'TERRAFORM: Wrong module' {
    run make -f ../tests/Makefile TERRAFORM_LINT_TARGET=./terraform/tf-unspecified-version terraform-lint
    assert_failure
}

@test 'TERRAFORM: Valid module' {
    run make -f ../tests/Makefile TERRAFORM_LINT_TARGET=./terraform/tf-good-module terraform-lint
    assert_success
}
